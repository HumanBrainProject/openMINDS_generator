...
class Expander(object):
    ...
    def expand():
        ...
        schema_root_path = os.path.join(root_path, "schemas")
        for schema_group in find_resource_directories(schema_root_path):
            ...
            absolute_schema_group_src_dir = schema_root_path if schema_group == '' else os.path.join(schema_root_path, schema_group)
            ...
            for schema_path in glob.glob(os.path.join(schema_root_path, schema_group, f'**/*{SCHEMA_FILE_ENDING}'), recursive=True):
                ...
                if TEMPLATE_PROPERTY_TYPE not in schema:
                    ...
                    Expander._process_schema(schema, schema_root_path)
    ...
    @staticmethod
    def _process_schema(schema, schema_root_path):
        if TEMPLATE_PROPERTY_EXTENDS in schema:
            extension_path = os.path.realpath(os.path.join(schema_root_path, schema[TEMPLATE_PROPERTY_EXTENDS]))
