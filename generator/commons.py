...
class Generator(object):
    ...
    def __init__(self, format, version):
        self.format = format
    ...
    def generate(self):
        ...
        for schema_group in find_resource_directories(expanded_path):
            ...
            for schema_path in glob.glob(os.path.join(schema_group_path, f'**/*{SCHEMA_FILE_ENDING}'), recursive=True):
                ...
                self._pre_process_template(schema, self.version)
                ...
                with open(target_file_path, "w") as target_file:
                    self._process_template(schema, target_file, self.version)
...
class JinjaGenerator(Generator):
    ...
    def __init__(self, format, autoescape, template_name, version):
        super().__init__(format, version)
